...
/*   Updated: 2025/03/14 10:55:36 by seerel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	images(t_game *game)
{
	...
	game->player = mlx_xpm_file_to_image(game->mlx, "images/plyer.xpm", &w, &h);
	if (game->player == NULL)
	...
	while (y < game->map_y)
	{
		while (x < game->map_x)
		{
			if (game->map[y][x] == '1')
				...
				mlx_put_image_to_window(game->mlx, game->win, game->coin_c, \
					x * 64, y * 64);
